...
/// Maximum and Minimum **absolute** values supported for [`Machine::max_travels`] for this module.
...
/// It is conventional to have **Z HOME_POS** above the part,
/// which requires that the machine can only travel in **negative Z** direction.
...
/// - [`CircularDirection`] for selecting a particular center point,
///   in case more than one circles are possible.
///
/// On success, returns a *tuple* consisting of two fields:
/// - The *center [`PlanarPoint`]* of the circle that satisfies the given conditions.
/// - The *radius* of the said circle
...
impl Display for MachineError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> Result<(), std::fmt::Error> {
        match self {
            Self::PositiveZ => write!(
                f,
                "Positive Z Axis Travel Detected:{RESET}\n\t\tPlease provide a valid negative value between: -{:?} and -{:?}",
                TRAVEL_LIMITS.max.z(),
                TRAVEL_LIMITS.min.z()
            ),
            Self::TravelsTooLong => write!(
                f,
                "Machine Travels Detected to be too long:{RESET}\n\t\tPlease provide values less than:\nX: {:?}\nY: {:?}\nZ: -{:?}",
                TRAVEL_LIMITS.max.x(),
                TRAVEL_LIMITS.max.y(),
                TRAVEL_LIMITS.max.z()
            ),
            Self::TravelsTooShort => write!(
                f,
                "Machine Travels Detected to be too short:{RESET}\n\t\tPlease provide values larger than:\nX: {:?}\nY: {:?}\nZ: -{:?}",
                TRAVEL_LIMITS.min.x(),
                TRAVEL_LIMITS.min.y(),
                TRAVEL_LIMITS.min.z()
            ),
            Self::WrongRatio => write!(
                f,
                "Unusual Machine Travels Ratio Detected:{RESET}\n\t\tPlease reconsider axis lengths."
            ),
            ...
            Self::CenterOffPlane => write!(
                f,
                "Invalid Relative Center Point Detected:{RESET}\n\t\tThe relative center provided does not lie on the selected plane, which is invalid."
            ),
            Self::OffsetDirection(dir) => match dir {
                Direction::Up | Direction::Down => write!(
                    f,
                    "Invalid Offset Direction Detected:{RESET}\n\t\tThe following direction was detected for a Diameter offset, which is invalid: {RED}{dir:?}{RESET}."
                ),
                Direction::Left | Direction::Right => write!(
                    f,
                    "Invalid Offset Direction Detected:{RESET}\n\t\tThe following direction was detected for a Height offset, which is invalid: {RED}{dir:?}{RESET}."
                ),
            },
        }
    }
