...
/// Same as [`Point`] but the fields are `Option`s.
#[derive(Clone, Default, Debug, PartialEq)]
...
/// - [`ParserError::UnexpectedPrefix`] -- A prefix was found, but cannot be parsed on its own.
pub fn parse(tokens: Vec<Token>) -> Result<Vec<Code>, ParserError> {
    ...
    // parse the rest of the prefixes

    for (prefix, suffix) in block.int_codes.drain() {
        codes.push(match prefix {
            b'N' => Code::N(suffix),
            b'O' => Code::O(suffix),
            b'S' => Code::S(suffix),
            b'T' => Code::T(suffix),
            _ => return Err(ParserError::UnexpectedPrefix(prefix)),
        });
    }

    for (prefix, suffix) in block.float_codes.drain() {
        codes.push(match prefix {
            b'F' => Code::F(suffix),
            b'X' => Code::X(suffix),
            b'Y' => Code::Y(suffix),
